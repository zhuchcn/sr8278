...
peakAnno_tx = annotatePeak(tx_granges, TxDb = TxDb.Hsapiens.UCSC.hg19.knownGene, annoDb = "org.Hs.eg.db", level = "gene")
peakAnno_ve = annotatePeak(ve_granges, TxDb = TxDb.Hsapiens.UCSC.hg19.knownGene, annoDb = "org.Hs.eg.db", level = "gene")
data.frame(
...
gene_df = left_join(gene_df, fpmk, by = "gene_id")
gene_df = filter(gene_df, X48h < con)
gene_df$fpmk_diff = gene_df$fpmk_con - gene_df$fpmk_48h
colnames(gene_df) = c("gene_id", "gene_name", "npeaks_ve", "npeaks_tx", "npeaks_diff", "fpmk_48h", "fpmk_con", "fpmk_diff")
write.table(gene_df, file = "gene_table_npeaks_fpmk.txt", sep = "\t", quote = FALSE, row.names = FALSE)
```
