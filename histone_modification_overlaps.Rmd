...
```{r}
rep1 = read.table("data/rep1SE_unstranded_anno_rsem.genes.results", sep = "\t", header = TRUE)
rep2 = read.table("data/rep2SE_unstranded_anno_rsem.genes.results", sep = "\t", header = TRUE)
fpkm = data.frame(
...
gene_df2 = left_join(gene_df, fpkm, by = "gene_id") %>% 
    filter(X48h < con & diff > 0) %>%
    mutate(fpkm_diff = con - X48h)
colnames(gene_df2) = c("gene_id", "gene_name", "npeaks_ve", "npeaks_tx", "npeaks_diff", "fpkm_48h", "fpkm_con", "fpkm_diff")
# write.table(gene_df2, file = "gene_table_npeaks_fpmk.txt", sep = "\t", quote = FALSE, row.names = FALSE)
```


Download the gene table with histone modification npeaks and RNAseq fpmk:

<a href="https://raw.githubusercontent.com/zhuchcn/sr8278/master/gene_table_npeaks_fpmk.txt" class="btn btn-primary">Download</a>

```{r}
gene_down = bitr(gene_df2$gene_id, fromType = "ENSEMBL", toType = "ENTREZID", OrgDb = org.Hs.eg.db, drop = TRUE)
gene_bg = bitr(gene_df$gene_id, fromType = "ENSEMBL", toType = "ENTREZID", OrgDb = org.Hs.eg.db, drop = TRUE)
ego = enrichGO(
    gene = gene_down$ENTREZID,
    OrgDb = org.Hs.eg.db,
...
write.table(ego@result, file = "go_enrichment_chipseq_vs_rnaseq.txt", sep = "\t", quote = FALSE, row.names = FALSE)
ego@result %>% dplyr::select(-ID, -geneID) %>% 
    DT::datatable() %>%
    DT::formatSignif(columns = 5:7, digits = 3)
```
...
<a href="https://raw.githubusercontent.com/zhuchcn/sr8278/master/go_enrichment_chipseq_vs_rnaseq.txt" class="btn btn-primary">Download</a>